...
def analysis(graphs, model, criterion_summed, device, num_classes, loader):
    ...
    W_norms = torch.norm(W.T, dim=0) # 类别均值矩阵和权重矩阵的范数分布特性
    ...
    graphs.Sw_invSb.append(np.trace(Sw @ inv_Sb))  # 类内协方差与类间协方差的比值，反映了分类边界的分离程度

    # ||W^T - M_||
    normalized_M = M_ / torch.norm(M_,'fro')
    normalized_W = W.T / torch.norm(W.T,'fro')
    graphs.W_M_dist.append((torch.norm(normalized_W - normalized_M)**2).item())   # 归一化后的权重矩阵与类别均值矩阵之间的距离，反映两者之间的相似性
    ...
    graphs.cos_W.append(coherence(W.T/W_norms))  #分别计算类别均值矩阵和权重矩阵的互相关性，衡量它们列向量之间的相似性
